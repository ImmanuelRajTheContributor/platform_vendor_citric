...
function setup_vendor() {
    ...
    export CITRIC_ROOT="$3"
    if [ ! -d "$CITRIC_ROOT" ]; then
        echo "\$CITRIC_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$CITRIC_ROOT/$OUTDIR" ]; then
        mkdir -p "$CITRIC_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$CITRIC_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$CITRIC_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$CITRIC_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$CITRIC_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        printf " * Copyright (C) $YEAR The CitricOS Project\n" >> $1
    elif [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
        printf " * Copyright (C) 2019-$YEAR The CitricOS Project\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR The CitricOS Project\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 The CitricOS Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR The CitricOS Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR The CitricOS Project\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The CitricOS Project\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR The CitricOS Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$CITRIC_ROOT"/prebuilts/tools-citric/common/smali/baksmali.jar
        export SMALIJAR="$CITRIC_ROOT"/prebuilts/tools-citric/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$CITRIC_ROOT"/prebuilts/tools-citric/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$CITRIC_ROOT"/prebuilts/tools-citric/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$CITRIC_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$CITRIC_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$CITRIC_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$CITRIC_ROOT"/vendor/citric/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$CITRIC_ROOT"/"$OUTDIR"/radio
